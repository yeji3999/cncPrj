  ...
  <div style="margin-top:15px">
    ...
    <smart-widget-grid :layout="layout" :resizable="false" :draggable="false">
      ...
      <smart-widget slot="2" simple>
        <div><span style="color:white"> dddd</span></div>
      </smart-widget>

      <smart-widget slot="5" simple>
      <div ref="chartTest" id="drag-elements" style="display:flex">
        <splitpanes class="default-theme" >
...
export default {
  components: {
    Header, Menu, Opsituation,Product,CycleTime,AnomalyData,Mae,ModalTP,ModalCT,ModalAdmin,TopMenu,ChartArea,RealTimeChart,Splitpanes,Pane
 ,    draggable
 },
  created() {
  ...
  mounted(){ const { chartTest } = this.$refs; 
  dragula([ chartTest, //어디서 어디로 옮기는지 
  ],
  ) } ,
  ...
  beforeUpdate(){
    ...
    if(this.hideOp && this.hideTp){
      this.hideOpTp = true
    }else{
      this.hideOpTp = false
    }

    if(this.hideMae && this.hideAnomaly){
      this.hideMaeAnomal = true
    }else{
       this.hideMaeAnomal = false     
    }
    console.log("a",a)
    ...
    if(this.hideMaeChart && this.hideRealtimeChart){
      this.chartLayout =  this.layout.slice(2,3)[0]
      console.log("ssssssssssssssssssss",this.chartLayout)
      this.layout.splice(2,1)
      b = false
    }else if(b==false){
      b=true
      this.layout.splice(1,0,this.chartLayout)
    }
  },
  updated(){
